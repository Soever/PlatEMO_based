...
function [Xfood, fval,gbest_t,Trajectories,fitness_history, position_history] = ESO_24(N,T,lb,ub,dim,fobj)
%% initial
vec_flag=[1,-1];
Threshold=0.25;
Thresold2= 0.6;
...
C3=zeros(1,T);
% for l= 1:T
%     C3(1,l)=2-2*sin(0.5*pi*(l/T)^4);%eq.(16) idea1
% end
C1=0.5;%eq.(14)
C2=0.05;%eq.(15)
for l= 1:T
    C3(1,l)=2;%eq.(16) idea1
end
ub1 = ub.*ones(1,dim);
lb1 = lb.*ones(1,dim);
if(max(size(ub)) == 1)
...
for t = 1:T
    ...
    for i=1:size(Positions,1)
     ...
     %% the principle of convex lens imaging idea2
