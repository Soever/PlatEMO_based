...
function [Xfood, fval,gbest_t,Trajectories,fitness_history, position_history] = ESO_AlgorithmFE_4_2(N,T,lb,ub,dim,fobj,stage)
...
for t = 1:T
    ...
    if Ybest1<Ybest2
        gbest_t(2*t-1)=min(Ybest1);
    else
        gbest_t(2*t-1)=min(Ybest2);
  ...
  BestIndex2=gbest2;
    ...
    if Ybest1<Ybest2
        gbest_t(2*t)=min(Ybest1);
    else
        gbest_t(2*t)=min(Ybest2);
