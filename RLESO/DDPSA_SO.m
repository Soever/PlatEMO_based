...
function [Xfood, fval,gbest_t ] = DDPSA_SO(N,T,lb,ub,dim,fobj)
%initial 
history = zeros(1,T);
vec_flag=[1,-1];
Threshold=0.25;
...
Xm_out = zeros(size(Xm));
Xf_out = zeros(size(Xf));
fitness_out_m=inf(size(fitness_m));
fitness_out_f=inf(size(fitness_f));
...
for t = 1:T
  ...
  for i = 1: length(indices)
        Xm(indices(i), :) = lb+rand*(ub-lb);
  end
  %Xm(indices, :) = Xm_out(indices, :);
  indices1 = find(failure_times_f >= maxFailure_times);
  Xf(indices1, :) = Xf_out(indices1, :);
  for i = 1: length(indices1)
        Xf(indices1(i), :) = lb+rand*(ub-lb);
  end
    if Q>1        Q=1;    end
...
if Q<Threshold
 ...
 hm = zeros(1,Nm);hf = zeros(1,Nm);
    for j=1:Nm
        ...
        hm(j) = y ; 
        if y<fitness_m(j)
            if fitness_m(j) < fitness_out_m(j)
                Xm_out(j,:) = Xm(j,:) ;
                fitness_out_m(j)=fitness_m(j);
            end
    ...
    for j=1:Nf
        ...
        y = feval(fobj,Xnewf(j,:));
         hf(j) = y ;
        if y<fitness_f(j)
             if fitness_f(j) < fitness_out_f(j)
                Xf_out(j,:) = Xf(j,:) ;
                fitness_out_f(j)=fitness_f(j);
            end
            fitness_f(j)=y;
            Xf(j,:)= Xnewf(j,:);
            failure_times_f(j) = 0; 
        else
            if y < fitness_out_f(j)
                Xf_out(j,:) = Xnewf(j,:) ;
                fitness_out_f(j)=y;
            end
