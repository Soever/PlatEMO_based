...
% reward 
function [Xfood, fval,gbest_t,Trajectories,fitness_history, position_history] = RLSO2_9(N,T,lb,ub,dim,fobj)
...
f0_f = mean(fitness_f)  ;
RF_num = 2 ;RD_num =2;strategy_num = 4 ;
...
for t = 1:T
    ...
    state_m = get_state_kmean(Xm,fitness_m,d0_m,f0_m,Diagonal_Length);
    state_f = get_state_kmean(Xf,fitness_f,d0_f,f0_f,Diagonal_Length);
    ...
    next_state_m = get_state_kmean(Xm,fitness_m,d0_m,f0_m,Diagonal_Length);
    next_state_f = get_state_kmean(Xf,fitness_f,d0_f,f0_f,Diagonal_Length);
...
function state = get_state_kmean(X,fitness,d0,f0,DL)
    [N,dim] = size(X) ;
    state = zeros(1,2) ;
    % diversity = cal_diversity(X) ;
    % D = sum(diversity) / (N*DL) ;
...
function [newX,fitness,reward] = Evaluation_reward(X,newX_dec,fitness,lb,ub,fobj)
    ...
    alpha = 0.5 ;
    reward  =alpha* (min(fitness_old ) -min(fitness_new))+ (1-alpha)*(mean(fitness_old )-mean(fitness_new))  ;
