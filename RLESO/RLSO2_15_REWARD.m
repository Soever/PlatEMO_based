...
% reward 
function [Xfood,fval,gbest_t] = RLSO2_15(N,T,lb,ub,dim,fobj)
...
for t = 1:T
    ...
    [Xm,fitness_m,reward_m] = Evaluation_reward(Xm,newXm_dec,fitness_m,lb,ub,fobj,t);
    [Xf,fitness_f,reward_f] = Evaluation_reward(Xf,newXf_dec,fitness_f,lb,ub,fobj,t);
...
function [newX,fitness,reward] = Evaluation_reward(X,newX_dec,fitness,lb,ub,fobj,t)
    N = size(X,1);
    fitness_new = zeros(size(fitness));
    fitness_old = fitness ;
...
%         fitness_new(j) = feval(fobj,newX_dec(j,:));
    ...
    reward =t* log(fitness_old./fitness_new) ;
