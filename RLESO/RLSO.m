...
% reward 
function [Xfood, fval,gbest_t,Trajectories,fitness_history, position_history] = RLSO(N,T,lb,ub,dim,fobj)
...
action_m = 3;
    ...
    [Xm,fitness_m] = Evaluation_reward(Xm,newXm_dec,fitness_m,lb,ub,fobj);
...
function [newX,fitness] = Evaluation_reward(X,newX_dec,fitness,lb,ub,fobj)
    N = size(X,1);
    fitness_new = zeros(size(fitness));
    fitness_old = fitness ;
    for j=1:N
