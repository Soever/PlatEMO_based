...
for Function_name = 1:25
    [lb,ub,dim,fobj] = Get_Functions_cec2017(Function_name,dim);
    lb = lb(1);ub = ub(1);
    for run = 1:runs
        [Best_score1,Best_pos1,cg_curve1]=ESO_Algorithm(nPop,Max_iter,lb,ub,dim,fobj);
        [Best_score2,Best_pos2,cg_curve2]=ESO_Algorithm(nPop,Max_iter,lb,ub,dim,fobj);
    end
end
